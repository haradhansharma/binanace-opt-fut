...
class FuturesFetcher:
    ...
    async def get_open_interest(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(data, '__dict__'):
                return {
                    "symbol": symbol,
                    "open_interest": float(getattr(data, 'open_interest', 0) or getattr(data, 'openInterest', 0) or 0),
    ...
    async def get_mark_price(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"symbol": symbol, "mark_price": 0.0, "index_price": 0.0, "time": datetime.utcnow()}
            ...
            for t in ticker_list:
                if hasattr(t, '__dict__'):
                    tickers.append({
                        ...
                        "price": float(getattr(t, 'last_price', 0) or getattr(t, 'lastPrice', 0) or 0),
                        "price_change": float(getattr(t, 'price_change', 0) or getattr(t, 'priceChange', 0) or 0),
                        "price_change_pct": float(getattr(t, 'price_change_percent', 0) or getattr(t, 'priceChangePercent', 0) or 0),
                        "high": float(getattr(t, 'high_price', 0) or getattr(t, 'highPrice', 0) or 0),
                        "low": float(getattr(t, 'low_price', 0) or getattr(t, 'lowPrice', 0) or 0),
                        "volume": float(getattr(t, 'volume', 0) or 0),
                        "quote_volume": float(getattr(t, 'quote_volume', 0) or getattr(t, 'quoteAssetVolume', 0) or 0),
                ...
                for s in data.symbols:
                    if hasattr(s, '__dict__'):
                        symbols_list.append({
                            ...
                            "base_asset": getattr(s, 'base_asset', '') or getattr(s, 'baseAsset', ''),
                            "quote_asset": getattr(s, 'quote_asset', '') or getattr(s, 'quoteAsset', ''),
                        })
