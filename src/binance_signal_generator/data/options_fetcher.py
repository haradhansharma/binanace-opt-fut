...
class OptionsFetcher:
    ...
    async def get_available_underlyings(self) -> List[str]:
        ...
        try:
            ...
            logger.info(
                "Fetched available underlyings", extra={"data": {"count": len(underlyings)}}
            )
            ...
            for expiry in sorted(expirations):
                ...
                for item in oi_list:
                    ...
                    if hasattr(item, "symbol"):
                        ...
                        oi_str = (
                            getattr(item, "sum_open_interest", None)
                            or getattr(item, "sumOpenInterest", None)
                            or "0"
                        )
                        try:
    ...
    async def get_block_trades(
        self, symbol: Optional[str] = None, limit: int = 500, use_cache: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for opt in option_symbols:
                ...
                if side == "CALL":
                    ...
                    total_call_volume += (
                        contract_volume  # BUG FIX (Bug #4): Use contract count, not USDT notional
                    )
                    total_call_notional += (
                        notional_volume  # BUG FIX (Bug #12): Track USDT notional separately
                    )
                elif side == "PUT":
    ...
    async def get_activity_summary(
        ...
        try:
            ...
            whale_activity = self._calculate_whale_activity(
                underlying, block_trades, chain.spot_price
            )
    ...
    def _calculate_whale_activity(
        self, underlying: str, block_trades: List[Dict[str, Any]], spot_price: float = 0.0
    ) -> float:
        """
        ...
        try:
            ...
            for item in items:
                if hasattr(item, "__dict__"):
                    timestamp_ms = int(
                        getattr(item, "expiry_date", 0) or getattr(item, "expiryDate", 0) or 0
                    )
                    result.append(
                        ExerciseRecord(
                            symbol=getattr(item, "symbol", ""),
                            strike_price=float(
                                getattr(item, "strike_price", 0)
                                or getattr(item, "strikePrice", 0)
                                or 0
                            ),
                            real_strike_price=float(
                                getattr(item, "real_strike_price", 0)
                                or getattr(item, "realStrikePrice", 0)
                                or 0
                            ),
                            expiry_date=datetime.fromtimestamp(timestamp_ms / 1000)
                            if timestamp_ms
                            else datetime.utcnow(),
                            strike_result=getattr(item, "strike_result", "")
                            or getattr(item, "strikeResult", "")
                            or "",
                        )
                    )
                elif isinstance(item, dict):
            ...
            logger.debug(
                ...
                extra={
                    "data": {
                        "underlying": underlying,
                        "exercised": sum(
                            1 for r in result if r.strike_result == "REALISTIC_VALUE_STRICKEN"
                        ),
                        "expired_otm": sum(
                            1 for r in result if r.strike_result == "EXTRINSIC_VALUE_EXPIRED"
                        ),
                    }
                },
            )
