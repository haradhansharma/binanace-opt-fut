...
from binance_signal_generator.config.loader import Config, OrchestratorConfig
...
class PipelineOrchestrator:
    ...
    def __init__(self, config: Config):
        ...
        Config is loaded from config.yaml via the main Config object.
        Single source of truth - orchestrator settings come from config.orchestrator.

        Args:
            config: Application configuration (loaded from config.yaml)
        """
        self.config = config
        # Get orchestrator config from main config (loaded from config.yaml)
        self.orchestrator_config = config.orchestrator

        # Initialize rate limiter from config.yaml (single source of truth)
        self.rate_limiter = RateLimiter(
            requests_per_second=float(config.binance.rate_limit_requests_per_second),
            burst=config.binance.rate_limit_burst,
        )
        ...
        # Initialize analysis modules - each loads its own config from config.yaml
        self.activity_scorer = ActivityScorer()
        self.asset_selector = AssetSelector()
        self.signal_scorer = SignalScorer()

        # Initialize whale detector - loads config from config.yaml
        self.whale_detector = WhaleDetector()

        # Initialize whale volume analyzer - loads config from config.yaml
        self.whale_volume_analyzer = WhaleVolumeAnalyzer()

        # Initialize wall detector - loads config from config.yaml
        self.wall_detector = WallDetector()
        self.sr_calculator = SRLevelCalculator()

        # Initialize gamma exposure calculator - loads config from config.yaml
        self.gamma_calculator = GammaExposureCalculator()

        # Initialize sentiment analyzer - loads config from config.yaml
        self.sentiment_analyzer = SentimentAnalyzer()

        # Initialize advanced analysis modules - each loads config from config.yaml
        ...
        logger.info(
            ...
            extra={
                "data": {
                    "top_n": self.orchestrator_config.top_n_assets,
                    "min_score": self.orchestrator_config.min_activity_score,
                }
            },
        )
    ...
    async def run(
        ...
        logger.info(
            f"Starting pipeline execution: {execution_id}", extra={"data": {"symbols": symbols}}
        )
        ...
        try:
            ...
            for i in range(0, min(len(underlyings), 50), batch_size):
                ...
                historical_results = await asyncio.gather(
                    *historical_data_tasks, return_exceptions=True
                )
                ...
                self._api_calls_made += (
                    len(batch) * 3
                )  # Options chain + OI history + Volume history
    ...
    async def _fetch_historical_data(self, symbol: str) -> tuple:
        ...
        try:
            ...
            if use_intraday:
                ...
                logger.debug(
                    f"Using intraday mode for {symbol}: oi_period={oi_period}, vol_interval={volume_interval}"
                )
            else:
                oi_period = "1d"
                oi_limit = 7
            ...
            if len(volume_history) >= 4:
                ...
                if use_intraday:
                    ...
                    avg_volume = (
                        sum(volumes[-17:-1]) / 16
                        if len(volumes) >= 17
                        else sum(volumes[:-1]) / max(1, len(volumes) - 1)
                    )
                else:
    ...
    async def _select_specific_symbols(
        ...
        """
        selected = []
        for i, symbol in enumerate(symbols[: self.orchestrator_config.top_n_assets]):
            selected.append(
                RankedAsset(
                    symbol=symbol,
                    rank=i + 1,
    ...
    async def _analyze_single_asset(
        ...
        try:
            ...
            whale_volume_analysis = self.whale_volume_analyzer.analyze(whale_trades, whale_analysis)
            ...
            iv_term_signal, iv_term_confidence, iv_term_details = (
                self.iv_analyzer.generate_term_structure_signal(iv_term_structure)
            )
            ...
            wall_sr_levels = (
                self.wall_detector.get_sr_from_walls(wall_analysis) if wall_analysis else ([], [])
            )
            ...
            if futures_data and futures_data.open_interest > 0:
                ...
                # Config-driven: use orchestrator config for thresholds
                oi_threshold = self.orchestrator_config.oi_threshold_daily  # Default from config
                intraday_config = getattr(self.config, "intraday", None)
                if intraday_config and intraday_config.enabled:
                    oi_threshold = (
                        self.orchestrator_config.oi_threshold_intraday
                    )  # Intraday from config
            ...
            logger.debug(
                ...
                f"min_required={self.orchestrator_config.min_signal_confidence}"
            )
            ...
            if options_signal.confidence >= self.orchestrator_config.min_signal_confidence:
                logger.info(
                    f"Signal generated for {symbol}: {options_signal.direction.value} ({options_signal.confidence:.2f})"
                )
                ...
                logger.debug(
                    f"Signal confidence {options_signal.confidence:.3f} below threshold {self.orchestrator_config.min_signal_confidence}"
                )
    ...
    def _create_trading_signal(
        ...
        if whale_analysis and whale_analysis.confidence_boost > 0:
            ...
            if whale_dir == "BULLISH" and signal_dir == SignalDirection.LONG:
                ...
                logger.debug(
                    f"Whale penalty: BULLISH whales oppose SHORT signal → -{penalty:.3f} confidence"
                )
                ...
                confidence = max(confidence - penalty, 0.0)
                logger.debug(
                    f"Whale penalty: BEARISH whales oppose LONG signal → -{penalty:.3f} confidence"
                )
            # else: whale_dir == "NEUTRAL" → no adjustment

        # Signal strength thresholds from config
        if confidence >= self.orchestrator_config.signal_strength_very_strong:
            strength = SignalStrength.VERY_STRONG
        elif confidence >= self.orchestrator_config.signal_strength_strong:
            strength = SignalStrength.STRONG
        elif confidence >= self.orchestrator_config.signal_strength_moderate:
        ...
        # Use config fallback when no data available
        atr_pct = self.orchestrator_config.atr_fallback_pct  # From config
        if hasattr(futures_data, "klines") and futures_data.klines:
            # Use recent candle range as ATR proxy
            recent_candles = (
                futures_data.klines[-14:] if len(futures_data.klines) >= 14 else futures_data.klines
            )
            if recent_candles:
                atr_values = [
                    (c.high - c.low) / c.close * 100 for c in recent_candles if c.close > 0
                ]
                if atr_values:
        ...
        # Max distances for intraday (from config)
        max_sl_distance_pct = self.config.output.stop_loss_max_distance_pct  # Default 3.0%
        max_level_distance_pct = self.orchestrator_config.max_level_distance_pct  # From config
            ...
            whale_metrics.update(
                {
                    ...
                    "whale_top_strike_concentration": round(
                        concentration.get("top_strike_concentration", 0), 3
                    ),
                    # Block trades
        ...
        options_metrics = {
            "pcr_combined": options_signal.pcr_analysis.pcr_combined
            if options_signal.pcr_analysis
            else 1.0,
            "iv_percentile": options_signal.iv_analysis.iv_percentile
            if options_signal.iv_analysis
            else 0.5,
            "max_pain_distance": options_signal.max_pain_analysis.distance_pct
            if options_signal.max_pain_analysis
            else 0.0,
                ...
                "top_strikes": [
                    {
                        "strike": s["strike"],
                        "pcr": round(s["pcr"], 3),
                        "distance_pct": s["distance_from_spot"],
                    }
        ...
        if oi_distribution:
            advanced_metrics["oi_distribution"] = {
                ...
                "top_oi_strikes": [
                    ...
                    for s in sorted(
                        oi_distribution.get("strikes", []),
                        key=lambda x: x.get("total_oi", 0),
                        reverse=True,
                    )[:5]
                ],
            }
        ...
        avg_tp_distance = (
            sum(tp.distance_pct for tp in take_profit_levels) / len(take_profit_levels)
                    ...
                    all_levels.append(
                        (
                            ...
                            wall.strength_score,
                            {
                                ...
                                "oi_concentration": round(wall.oi_percentage * 100, 2),
                            },
                        )
                    )
    ...
    def _calculate_smart_take_profit(
        ...
        """
        levels = []
        max_tp_distance_pct = self.orchestrator_config.max_tp_distance_pct  # From config
                ...
                if 0 < tp_distance <= max_tp_distance_pct:
                    tp_candidates.append(
                        (
                            ...
                            res.get("gex", res.get("oi", 0)) / 10000
                            if res.get("gex")
                            else res.get("oi", 0) / 100,
                        )
                    )
                ...
                if 0 < tp_distance <= max_tp_distance_pct:
                    tp_candidates.append(
                        (
                            ...
                            sup.get("gex", sup.get("oi", 0)) / 10000
                            if sup.get("gex")
                            else sup.get("oi", 0) / 100,
                        )
                    )
        ...
        for i, candidate in enumerate(unique_candidates[:3]):
            ...
            sl_distance = (
                stop_loss.distance_pct if stop_loss and stop_loss.distance_pct > 0 else 2.0
            )
    ...
    def _filter_signals(
        ...
        for signal in signals:
            # Check confidence
            if signal.confidence_score < self.orchestrator_config.min_signal_confidence:
                continue
            ...
            if len(filtered) >= self.orchestrator_config.max_signals_per_run:
                break
