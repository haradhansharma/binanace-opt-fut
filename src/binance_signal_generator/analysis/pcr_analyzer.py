...
from binance_signal_generator.config import load_config, PCRAnalyzerConfig
...
class PCRAnalyzer:
    ...
    Attributes:
        config: PCR analysis configuration (loaded from config.yaml)
    """
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config = loaded_config.analysis.pcr_analyzer_config
    ...
    def analyze(
        ...
        if total_oi < self.config.min_total_oi:
            logger.warning(f"Insufficient OI for PCR analysis: {total_oi}")
    ...
    def analyze_pcr_by_strike(
        ...
        put_heavy_strikes = [s for s in strike_pcrs if s["pcr"] >= self.config.pcr_high_threshold]
        call_heavy_strikes = [s for s in strike_pcrs if s["pcr"] <= self.config.pcr_low_threshold]

        return {
            "strike_pcrs": strike_pcrs,
            "put_heavy_count": len(put_heavy_strikes),
            "call_heavy_count": len(call_heavy_strikes),
            "avg_pcr": sum(s["pcr"] for s in strike_pcrs if s["pcr"] < 10)
            / max(1, len([s for s in strike_pcrs if s["pcr"] < 10])),
        }
