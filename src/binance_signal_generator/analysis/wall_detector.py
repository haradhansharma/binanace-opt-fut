...
from binance_signal_generator.config import load_config, WallDetectorAnalyzerConfig
...
class WallDetector:
    ...
    Attributes:
        config: Wall detection configuration (loaded from config.yaml)
    """
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config = loaded_config.analysis.wall_detector_config
    ...
    def detect(self, chain: OptionsChain) -> WallAnalysis:
        ...
        if total_oi < self.config.min_absolute_oi:
            logger.warning(
                f"Insufficient total OI for wall detection: {total_oi} < {self.config.min_absolute_oi}"
            )
    ...
    def get_wall_summary(self, analysis: WallAnalysis) -> Dict[str, Any]:
        ...
        return {
            ...
            "nearest_support": analysis.nearest_put_wall.strike
            if analysis.nearest_put_wall
            else None,
            "nearest_resistance": analysis.nearest_call_wall.strike
