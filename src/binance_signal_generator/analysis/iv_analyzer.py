...
class IVAnalyzer:
    ...
    Attributes:
        config: IV analysis configuration (loaded from config.yaml)
    """
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        All IV settings come from config.analysis.iv section.
        ...
        self.config = loaded_config.analysis.iv_config

        # Rolling IV history for adaptive percentile calculation
        self._iv_history = []
        ...
        logger.info(
            ...
            extra={
                "data": {
                    ...
                    "iv_high_value": self.config.iv_high_value,
                    "iv_low_value": self.config.iv_low_value,
                    "atm_range_pct": self.config.atm_range_pct,
                    "min_strikes": self.config.min_strikes,
                }
            },
        )
    ...
    def analyze(self, chain: OptionsChain) -> IVAnalysis:
        ...
        if len(chain.strikes) < self.config.min_strikes:
            logger.warning(f"Insufficient strikes for IV analysis: {len(chain.strikes)}")
        ...
        if shape == "INVERTED":
            ...
            if inversion_ratio > 0.3:
                ...
                details["interpretation"] = (
                    "Strongly inverted - major near-term volatility expected"
                )
    ...
    def analyze_term_structure_from_chain(
        ...
        avg_iv = (
            (chain.avg_call_iv + chain.avg_put_iv) / 2
            if (chain.avg_call_iv > 0 and chain.avg_put_iv > 0)
            else self._calculate_atm_iv(chain)
        )
