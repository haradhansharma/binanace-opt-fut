...
from binance_signal_generator.config import load_config, AssetSelectorConfig
...
class AssetSelector:
    ...
    Attributes:
        config: Selection configuration (loaded from config.yaml)
    """
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config = loaded_config.asset_selector
    ...
    def select(
        ...
        for rank, metrics in enumerate(sorted_metrics, 1):
            ...
            if metrics.activity_score < self.config.min_activity_score:
                logger.debug(f"{metrics.symbol} below threshold: {metrics.activity_score:.2f}")
                continue
    ...
    def _check_liquidity(self, metrics: ActivityMetrics) -> bool:
        ...
        """
        return (
            metrics.total_options_volume >= self.config.min_options_volume
            and metrics.num_strikes_active >= self.config.min_active_strikes
        )
