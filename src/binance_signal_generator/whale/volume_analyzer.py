...
from binance_signal_generator.config import load_config, VolumeAnalyzerConfig
...
class WhaleVolumeAnalyzer:
    ...
    Attributes:
        config: Volume analyzer configuration (loaded from config.yaml)
    """
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config = loaded_config.volume_analyzer
    ...
    def analyze(
        ...
        return {
            ...
            "flow": flow,
            "block_trades": blocks,
            "summary": self._create_summary(time_analysis, concentration, flow, blocks),
        }
    ...
    def _analyze_time_pattern(
        ...
        for i in range(self.config.time_buckets):
            ...
            bucket_trades = [t for t in sorted_trades if bucket_start <= t.timestamp < bucket_end]
    ...
    def _analyze_flow(
        ...
        """
        call_buy = sum(
            t.premium for t in whale_trades if t.option_type == "CALL" and t.direction == "BUY"
        )
        call_sell = sum(
            t.premium for t in whale_trades if t.option_type == "CALL" and t.direction == "SELL"
        )
        put_buy = sum(
            t.premium for t in whale_trades if t.option_type == "PUT" and t.direction == "BUY"
        )
        put_sell = sum(
            t.premium for t in whale_trades if t.option_type == "PUT" and t.direction == "SELL"
        )
        ...
        return {
            ...
            "call_put_ratio": (call_buy + call_sell) / (put_buy + put_sell)
            if (put_buy + put_sell) > 0
            else 1.0,
            "aggressive_side": "BUYERS"
            if (call_buy + put_buy) > (call_sell + put_sell)
            else "SELLERS",
        }
