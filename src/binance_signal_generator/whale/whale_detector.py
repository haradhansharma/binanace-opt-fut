...
from binance_signal_generator.config import load_config, WhaleDetectorAnalyzerConfig, WhaleConfig
...
class WhaleDetector:
    ...
    Attributes:
        config: Whale detection configuration (loaded from config.yaml)
        whale_config: Whale config from main config (for asset thresholds)
    """
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config = loaded_config.whale_detector
        self.whale_config = loaded_config.whale
        ...
        logger.info(
            ...
            extra={
                "data": {
                    ...
                    "asset_specific_count": len(self.whale_config.asset_thresholds)
                    if self.whale_config.asset_thresholds
                    else 0,
                }
            },
        )
    ...
    def _get_thresholds_for_asset(self, symbol: str) -> Tuple[float, float]:
        ...
        """
        # Check for asset-specific thresholds in whale_config (not whale_detector config)
        if self.whale_config.asset_thresholds and symbol in self.whale_config.asset_thresholds:
            threshold = self.whale_config.asset_thresholds[symbol]
            return threshold.min_premium, threshold.block_threshold

        # Return defaults from whale_detector config
    ...
    def _filter_whale_trades(
        ...
        min_premium, block_threshold = (
            self._get_thresholds_for_asset(symbol)
            if symbol
            else (self.config.min_premium, self.config.block_threshold)
        )
    ...
    def _parse_trade(
        self, trade: Dict[str, Any], block_threshold: Optional[float] = None
    ) -> Optional[WhaleTrade]:
        """
        ...
        try:
            ...
            threshold = (
                block_threshold if block_threshold is not None else self.config.block_threshold
            )
    ...
    def analyze_block_trades(
        ...
        underlying_trades = [t for t in block_trades if t.get("symbol", "").startswith(f"{base}-")]
        ...
        if underlying_trades:
            logger.debug(
                f"Sample block trade for {options_chain.underlying}: {underlying_trades[0]}"
            )
    ...
    def parse_block_trades_to_whale_trades(
        ...
        logger.debug(f"Parsed {len(whale_trades)} whale trades for {underlying}")
